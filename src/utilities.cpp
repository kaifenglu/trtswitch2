...
void chinv2(FlatMatrix& matrix, int n) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (mii > 0.0) {
      ...
      for (int j = i + 1; j < n; ++j) {
        std::size_t idx_ij = FlatMatrix::idx_col(static_cast<std::size_t>(i), static_cast<std::size_t>(j), static_cast<std::size_t>(n));
...
ListCpp qrcpp(const FlatMatrix& X, double tol) {
  ...
  while (tau > tol) {
    ...
    if (msub > 0 && r < static_cast<int>(n)) row_house(A, r, static_cast<int>(m) - 1, r, static_cast<int>(n) - 1, v);
    ...
    for (int j = r + 1; j < static_cast<int>(n); ++j) {
      double val = A.data[FlatMatrix::idx_col(static_cast<std::size_t>(r), static_cast<std::size_t>(j), m)];
      cvec[static_cast<size_t>(j)] -= val * val;
      if (cvec[static_cast<size_t>(j)] < 0.0) cvec[static_cast<size_t>(j)] = 0.0;
    }
    
    if (r < static_cast<int>(n) - 1) tau = max_elem(cvec, r + 1, static_cast<int>(n) - 1);
    else tau = 0.0;
  } // end while
  ...
  if (r >= 0) {
    for (int kk = r; kk >= 0; --kk) {
      ...
      for (int jj = kk; jj < static_cast<int>(m); ++jj) {
        double acc = 0.0;
        for (int ii = 0; ii < msub_k; ++ii) {
          acc += Qf.data[FlatMatrix::idx_col(static_cast<std::size_t>(kk + ii), static_cast<std::size_t>(jj), m)] * vks[static_cast<size_t>(ii)];
        }
        w[static_cast<size_t>(jj - kk)] = acc * (-2.0 / sumsq(vks));
      }
      for (int ii = 0; ii < msub_k; ++ii) {
        for (int jj = kk; jj < static_cast<int>(m); ++jj) {
          std::size_t idx = FlatMatrix::idx_col(static_cast<std::size_t>(kk + ii), static_cast<std::size_t>(jj), m);
